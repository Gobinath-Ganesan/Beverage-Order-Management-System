...
public class DB {
    ...
    public static HashMap<Integer,Bottle> bottleInfo = new HashMap<Integer,Bottle>();

    public static HashMap<Integer,Crate> crateInfo = new HashMap<Integer,Crate>();

    public static HashMap<Integer,Order> orderInfo = new HashMap<Integer, Order>();
    ...
    private List<Bottle> initBottles() {
        return new ArrayList<>(Arrays.asList(
                ...
                new Bottle(5, "Gin", 0.5, true, 42.00, 13.79, "Hopfengarten", 3),
    ...
    public int bootleId(List<Bottle> argAvailableBottles){

        int botid = argAvailableBottles.stream().map(Bottle::getId).max(Comparator.naturalOrder()).orElse(0) + 1;
        return botid;
    }

    public int crateId(List<Crate> argAvailableCrates){
        int cratid = argAvailableCrates.stream().map(Crate::getId).max(Comparator.naturalOrder()).orElse(0) + 1;
        return cratid;
    }

    public int orderId(List<Order> argAvailableOrder){
        int orderId = argAvailableOrder.stream().map(Order::getOrderId).max(Comparator.naturalOrder()).orElse(0) + 1;
        return orderId;
    }

    public void updateBottleInfo(){
        for (Bottle b:bottles
             ) {
            bottleInfo.put(b.getId(),b);
        }
    }

    public void updateCrateinfo(){
        for (Crate c:crates
        ) {
            crateInfo.put(c.getId(),c);
        }
    }
